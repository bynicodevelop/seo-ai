...
import type { Article, Category } from '~/functions/src/shared';
const runtimeConfig = useRuntimeConfig();
const { $domain } = useNuxtApp() as any;
...
try {
    const { data: latestArticles } = await useAsyncData<{
        article: Article,
        category: Category
    }[]>('latestArticles', async () => await fetchLatestArticles($domain as string, runtimeConfig.public.limitArticles));
